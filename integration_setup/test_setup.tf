...
output "aws_go_forward_command" {
  value       = <<EOT
  ## Run:
./build/aws-go-forward --profile default --instance-name ${aws_instance.ec2.tags["Name"]} --local-port 3306 --remote-host ${aws_db_instance.mariadb.address} --remote-port 3306 --region ${var.region}
 ## Or use the generated config file:
./build/aws-go-forward --config integration_setup/aws-go-forward.ini
 ## And:
